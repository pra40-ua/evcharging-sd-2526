...
python -c "import mysql.connector; conn = mysql.connector.connect(host='!CENTRAL_IP_BD!', port=3306, user='root', password='root', database='evcharging'); cursor = conn.cursor(); cursor.execute('SHOW TABLES'); tables = cursor.fetchall(); print('Tablas en la base de datos evcharging:'); print(''); for table in tables: print(f'  - {table[0]}'); cursor.close(); conn.close()"
