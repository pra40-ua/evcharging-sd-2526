...
REM ========================================================================
REM  Este script crea las tablas faltantes en MariaDB
REM ========================================================================
...
REM Verificar que el contenedor MariaDB existe
docker ps -a | findstr mariadb >nul 2>&1
if %errorlevel% neq 0 (
    echo [ERROR] El contenedor MariaDB no existe.
    echo.
...
REM Verificar que MariaDB está corriendo
docker ps | findstr mariadb >nul 2>&1
if %errorlevel% neq 0 (
    echo [ERROR] El contenedor MariaDB existe pero no esta corriendo.
    echo.
    echo Ejecuta: docker start mariadb
    pause
    exit /b 1
)

echo [1/2] Verificando conexion a MariaDB...
echo.

docker exec mariadb mariadb-admin ping -h localhost -uroot -proot >nul 2>&1
if %errorlevel% neq 0 (
    ...
    docker exec mariadb mariadb-admin ping -h localhost -uroot -proot >nul 2>&1
    if %errorlevel% neq 0 (
        echo [ERROR] MariaDB sigue sin responder.
        pause
        exit /b 1
    )
)

echo [OK] MariaDB esta activo y respondiendo
echo.
...
echo.
docker exec mariadb mariadb -u root -proot evcharging -e "SHOW TABLES;"
