...
Write-Host "  - Puerto Registry: 8000"
Write-Host ""
...
if (Test-Path $certPath) {
    if (Test-Path $keyPath) {
        ...
        Write-Host ""
        Start-Process -FilePath "python" -ArgumentList "ev_registry\EV_Registry.py", "--db-host", $CENTRAL_IP_BD, "--db-port", "3306", "--db-user", "root", "--db-password", "root", "--db-name", "evcharging", "--port", "8000", "--ssl", "--ssl-cert", $certPath, "--ssl-key", $keyPath -WindowStyle Normal
        Write-Host "[OK] EV_Registry iniciado con HTTPS (puerto 8000)" -ForegroundColor Green
        Write-Host "  - API REST: https://localhost:8000/api" -ForegroundColor Cyan
    } else {
        Write-Host "[INFO] Certificado encontrado pero falta la clave. Iniciando con HTTP..." -ForegroundColor Yellow
        Start-Process -FilePath "python" -ArgumentList "ev_registry\EV_Registry.py", "--db-host", $CENTRAL_IP_BD, "--db-port", "3306", "--db-user", "root", "--db-password", "root", "--db-name", "evcharging", "--port", "8000" -WindowStyle Normal
        Write-Host "[OK] EV_Registry iniciado con HTTP (puerto 8000)" -ForegroundColor Green
        Write-Host "  - API REST: http://localhost:8000/api" -ForegroundColor Cyan
    }
} else {
